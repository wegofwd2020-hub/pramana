...
| 3 | **SIGILL mid-encode**, after all 219 T5 shards load. Attributed at the time to `transformers 5.17.0`, on the strength of one crashing venv and one working one. **That attribution is retracted** — `5.16.1` has since SIGILLed too. The core dump's stack sits in `libtorch_cpu.so` on an OpenMP worker thread, on a CPU with AVX but no AVX2/FMA, while torch's own dispatch reports `DEFAULT`. Intermittent, ~1 run in 5. See wegofwd-video's `docs/local-diffusion-cpu-poc.md`. |
...
That is a compliance property, not a cost one.

The `render` extra pins `transformers==5.16.1` because that is what this run
completed on — a known-good combination, which is what reproducibility needs. It
is **not** a fix for the SIGILL above, which is intermittent and appears to come
from torch's CPU kernels. Bumping the pin is fine; re-verify with a full render
rather than an import, because the failure happens six minutes in.
